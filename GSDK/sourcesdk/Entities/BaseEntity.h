...
public:
	...
	*/
	...
	{
		CBaseHandle* hActiveWeapon = (CBaseHandle*)((uintptr_t)this + offsets::DT_BaseCombatCharacter::m_hActiveWeapon);
		
		if (!hActiveWeapon)
			return nullptr;

		return (CBaseCombatWeapon*)(cliententitylist()->GetClientEntityFromHandle(hActiveWeapon));
		//using GetActiveWeaponFn = CBaseCombatWeapon*(__thiscall*)(void*);
		//static GetActiveWeaponFn GetActiveWeapon;
		////8B 91 ? ? ? ? 83 FA FF 74 1D A1 ? ? ? ? 8B CA 81 E1 ? ? ? ? C1 EA 0E 03 C9 39 54 C8 08 75 05 8B 44 C8 04 C3 33 C0 C3
		//if (!GetActiveWeapon)
		//	GetActiveWeapon = (GetActiveWeaponFn)(Util::Pattern::FindPattern("client.dll", "8B 91 ? ? ? ? 83 FA FF 74 1D A1 ? ? ? ? 8B CA 81 E1 ? ? ? ? C1 EA 0E 03 C9 39 54 C8 08 75 05 8B 44 C8 04 C3 33 C0 C3", 5));
		//if (!GetActiveWeapon)
		//	return NULL;
		//
		//return GetActiveWeapon(this);
	}
	...
	//CBaseCombatWeapon* GetActiveWeapon()
	//{
	//	CBaseCombatWeapon* hActiveWeapon = call_virtual<342, CBaseCombatWeapon*>(this);
	//
	//	return hActiveWeapon;
	//}
	BaseEntity* GetOwner()
	{
		CBaseHandle* howner = (CBaseHandle*)((uintptr_t)this + offsets::DT_BaseEntity::m_hOwnerEntity);
		if (!howner)
			return nullptr;
	...
	BaseEntity* GetHands()
	{
		CBaseHandle* m_Hands = (CBaseHandle*)((uintptr_t)this + offsets::DT_GMOD_Player::m_Hands);
		if (!m_Hands)
			return nullptr;
