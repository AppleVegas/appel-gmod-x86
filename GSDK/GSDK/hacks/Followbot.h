namespace H
{
	...
	void RunFollowbot(CUserCmd* cmd)
	{
		...
		for (int i = 1; i <= engine()->GetMaxClients(); i++)
		{
			...
			if (HackVars::Script::FollowBot::Key)
			{
				if (!IsKeyDown(HackVars::Script::FollowBot::Key))
					return;
			}
			...
			if (pLocal->m_vecOrigin().DistTo(pEntity->m_vecOrigin()) > (float)HackVars::Script::FollowBot::MaxDist)
				continue;

			if (pLocal->m_vecOrigin().DistTo(pEntity->m_vecOrigin()) < (float)HackVars::Script::FollowBot::MinDist)
				continue;
