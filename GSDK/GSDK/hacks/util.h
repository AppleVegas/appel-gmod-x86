...
namespace H
{
	namespace Util
	{
		namespace Math
		{
			...
			float AngleNormalizeA(float angle)
			{
				angle = fmodf(angle, 360.0f);
				if (angle > 360)
				{
		...
		bool IsNospreadWeapon()
		{
			...
			*/
			...
			if (!strcmp(name, "weapon_357") || !strcmp(name, "weapon_crossbow") || !strcmp(name, "weapon_crowbar") || !strcmp(name, "weapon_stunstick") || !strcmp(name, "weapon_shotgun"))
				return true;
